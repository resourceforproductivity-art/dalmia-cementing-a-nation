    ...
    <main>
      ...
      <Reveals>
        ...
        <section id="our-world" className="world-section section-shell" aria-labelledby="world-title">
          ...
          <div className="world-grid">{worlds.map(world => <article id={world.id} key={world.id} className={`world-panel panel-${world.id}`} data-reveal><div className="panel-image"><Image src={assetPath(world.image)} alt={world.alt} fill sizes="(max-width: 767px) 90vw, (max-width: 1023px) 45vw, 33vw" /><div className="panel-shade" /><span className="panel-index eyebrow">{world.number} /</span><h3>{world.name}</h3></div><p>{world.line}</p></article>)}</div>
        </section>
