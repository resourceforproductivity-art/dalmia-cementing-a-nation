    ...
    <main>
      ...
      <Reveals>
        <section id="about" className="about-section section-shell" aria-labelledby="about-title">
          ...
          <div className="about-heading" data-reveal><h2 id="about-title">Building<br /><em>Beyond Cement.</em></h2><span className="architectural-cross" aria-hidden="true">+</span></div>
          <div className="about-body">
            <div className="about-image-wrap" data-reveal><figure className="about-image"><Image src="/images/film-finale.jpg" alt="Sculptural industrial architecture from the Cementing a Nation film" fill sizes="(max-width: 767px) 90vw, 48vw" /><span className="image-corner corner-tl" /><span className="image-corner corner-br" /></figure><div className="image-caption eyebrow"><span>A VISION MADE TANGIBLE</span><span>DALMIA BHARAT</span></div></div>
            <div className="about-copy" data-reveal><span className="copy-rule" /><p className="about-lead">Some foundations<br />hold more than buildings.</p><p>They hold ambition. They connect communities. They give shape to what a nation can become.</p><p>At Dalmia Bharat, the story of cement is part of a larger story — of people, possibility and the resolve to keep moving forward.</p><a className="round-link dark-link" href="#our-world">DISCOVER OUR WORLD<span><Arrow /></span></a></div>
          </div>
        ...
        <section id="our-world" className="world-section section-shell" aria-labelledby="world-title">
          ...
          <div className="world-grid">{worlds.map(world => <article id={world.id} key={world.id} className={`world-panel panel-${world.id}`} data-reveal><div className="panel-image"><Image src={world.image} alt={world.alt} fill sizes="(max-width: 767px) 90vw, (max-width: 1023px) 45vw, 33vw" /><div className="panel-shade" /><span className="panel-index eyebrow">{world.number} /</span><h3>{world.name}</h3></div><p>{world.line}</p></article>)}</div>
        </section>
